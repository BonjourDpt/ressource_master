...
- **After pulling schema changes:** run `npm run prisma:migrate` so new tables such as `ResourceTimeOff` are applied locally, then regenerate the Prisma client if prompted.
